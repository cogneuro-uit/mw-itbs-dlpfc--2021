...
We conducted an exploratory Bayesian analysis to determine the relationship between AE, BV and MW. We used an ordered-probit model, treating MW as the outcome variable and including the behavioural indices behavioural variability and approximate entropy along with block, stimulation condition and all relevant interactions (see Table S8 for all coefficients). Also in this model, we found that real stimulation increased MW, but for the last block (B1 $\times$ stimulation: `r coef_hdi(llm[,"b_stimulationreal:blockB1"])`; B2 $\times$ stimulation: `r coef_hdi(llm[,"b_stimulationreal:blockB2"])`; B3 $\times$ stimulation: `r coef_hdi(llm[,"b_stimulationreal:blockB3"])`), compared to sham stimulation (B1: `r coef_hdi(llm[,"b_blockB1"])`; B2: `r coef_hdi(llm[,"b_blockB2"])`; B3: `r coef_hdi(llm[,"b_blockB3"])`). Intriguingly, we found that real stimulation interacted with BV during the last two blocks (BV $\times$ B2 $\times$ stimulation: `r coef_hdi(llm[,"b_zlogbv:stimulationreal:blockB2"])`; BV $\times$ B3 $\times$ stimulation: `r coef_hdi(llm[,"b_zlogbv:stimulationreal:blockB3"])`), such that BV no longer predicted MW (see Figure **XX**). A similar pattern was observed for AE, in the sham condition, the negative relationship to MW in the baseline block (AE: `r coef_hdi(llm[,"b_zlogapen"])`) became weaker through repeated iTBS (AE $\times$ B1: `r coef_hdi(llm[,"b_zlogapen:blockB1"])`; AE $\times$ B2: `r coef_hdi(llm[,"b_zlogapen:blockB2"])`; AE $\times$ B3: `r coef_hdi(llm[,"b_zlogapen:blockB3"])` ). Similarly, the real stimulation condition indicated the same pattern albeit a weaker negative prediction of MW in the baseline block (AE $\times$ stimulation: `r coef_hdi(llm[,"b_zlogbv:stimulationreal"])`; AE $\times$ B1 $\times$ stimulation: `r coef_hdi(llm[,"b_zlogbv:stimulationreal:blockB1"])`; AE $\times$ B2 $\times$ stimulation: `r coef_hdi(llm[,"b_zlogbv:stimulationreal:blockB2"])`; AE $\times$ B3 $\times$ stimulation: `r coef_hdi(llm[,"b_zlogbv:stimulationreal:blockB3"])`). 
...
#colnames(llm)
as_tibble(llm) |>
  pivot_longer(c("b_zlogbv:blockB1","b_zlogbv:blockB2","b_zlogbv:blockB3")) |>
  ggplot(aes(name, value)) +
  stat_summary(fun.data = mean_se)

as_tibble(llm) |>
  pivot_longer(c("b_zlogbv:stimulationreal:blockB1","b_zlogbv:stimulationreal:blockB2","b_zlogbv:stimulationreal:blockB3")) |>
  ggplot(aes(name, value)) +
  stat_summary(fun.data = mean_se)
```
...
as_tibble(llm) |>
  mutate(
    B0_BV_sham = b_zlogbv,
    B0_BV_real = `b_zlogbv:stimulationreal`,
    B1_BV_sham = b_zlogbv+`b_zlogbv:blockB1`,
    B1_BV_real =`b_zlogbv:stimulationreal`+`b_zlogbv:blockB1` + `b_zlogbv:stimulationreal:blockB1`,
    B2_BV_sham = b_zlogbv+`b_zlogbv:blockB2`,
    B2_BV_real = `b_zlogbv:stimulationreal`+`b_zlogbv:blockB2` + `b_zlogbv:stimulationreal:blockB2`,
    B3_BV_sham = b_zlogbv+`b_zlogbv:blockB3`,
    B3_BV_real = `b_zlogbv:stimulationreal`+`b_zlogbv:blockB3` + `b_zlogbv:stimulationreal:blockB3`,
...
as_tibble(llm) |>
  mutate(
    B0_BV_sham = 0,
    B0_BV_real = 0,
    B1_BV_sham = `b_zlogbv:blockB1`,
    B1_BV_real = `b_zlogbv:blockB1` + `b_zlogbv:stimulationreal:blockB1`,
    B2_BV_sham = `b_zlogbv:blockB2`,
    B2_BV_real = `b_zlogbv:blockB2` + `b_zlogbv:stimulationreal:blockB2`,
    B3_BV_sham = `b_zlogbv:blockB3`,
    B3_BV_real = `b_zlogbv:blockB3` + `b_zlogbv:stimulationreal:blockB3`,
    B0_AE_sham = 0,
    B0_AE_real = 0,
...
as_tibble(llm) |>
  mutate(
    B0_BV_diff = 0,
    B1_BV_diff = `b_zlogbv:stimulationreal:blockB1` - `b_zlogbv:blockB1`,
    B2_BV_diff = `b_zlogbv:stimulationreal:blockB2` - `b_zlogbv:blockB2`,
    B3_BV_diff = `b_zlogbv:stimulationreal:blockB3` - `b_zlogbv:blockB3`,
    B0_AE_diff = 0,
